...
public interface TotalOrderMapper {
    ...
    // 支付时调用
    @Update("UPDATE total_order SET status = 'PAID', payTime = NOW() WHERE id = #{id}")
    ...
    @Update("UPDATE total_order SET status = 'CANCELLED', finish_time = NOW() WHERE id = #{id}")
    int cancelOrder(@Param("id") int id);



    // 在时间到达后自动调用（暂时不知道怎么用）
    @Update("UPDATE total_order SET status = 'FINISHED', finish_time = NOW() WHERE id = #{id}")
